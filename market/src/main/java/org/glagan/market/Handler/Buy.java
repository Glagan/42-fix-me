...
public class Buy extends Handler {
    ...
    public boolean handle(Client client, Message message) {
        if (message.getHeader().getMsgType().equals(MsgType.Buy)) {
            ...
            System.out.println("Transaction procesed");
            Message executed = Message.make(MsgType.Executed)
                    .auth(client.getId())
                    .continueFrom(1)
                    .add(Dictionary.Broker, message.getHeader().getBeginString())
                    .add(Dictionary.OrderId, message.getBody().get(Dictionary.OrderId))
                    .add(Dictionary.Market, message.getBody().get(Dictionary.Market))
                    .add(Dictionary.Instrument, message.getBody().get(Dictionary.Instrument))
                    .add(Dictionary.Quantity, message.getBody().get(Dictionary.Quantity))
                    .add(Dictionary.Price, message.getBody().get(Dictionary.Price))
                    .build();
